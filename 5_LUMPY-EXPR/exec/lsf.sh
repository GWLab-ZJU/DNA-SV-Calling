...
LIBDO_LOG_MODE=2
[ -f "${tdc}" ] || DO samtools view -b -F 1294 "${tdm}" \| samtools sort -o "${tdc}" -
[ -f "${tds}" ] || DO samtools view -h "${tdm}" \| "${LUMPYscripts}"/extractSplitReads_BwaMem -i stdin \| samtools view -Sh -\|samtools sort -o "${tds}" -
LIBDO_LOG_MODE=4
